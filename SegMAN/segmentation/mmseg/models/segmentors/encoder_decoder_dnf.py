...
class EncoderDecoderDNF(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        # Fix: call BaseSegmentor init via correct class
        super(EncoderDecoderDNF, self).__init__(init_cfg)
    ...
    def extract_feat(self, img, **kwargs):
        ...
        Expect depth/normal to be provided as tensors in kwargs when available.
        No per-sample file reading is performed here.
        """
        x = self.backbone(img)
        if self.with_neck:
            has_dn_tensor = ('depth' in kwargs and 'normal' in kwargs)
            if has_dn_tensor:
                # Ensure dtype/device consistency with backbone features (esp. AMP)
                feat0 = x[0] if isinstance(x, (list, tuple)) else x
                target_dtype = feat0.dtype
                ...
                try:
                    ...
                    # Fallback for necks that only accept tensor/list inputs
                    x = self.neck(x)
            else:
                x = self.neck(x)
        return x
    ...
    def simple_test_logits(self, img, img_metas, rescale=True, **kwargs):
        """Test without augmentations.

        Return numpy seg_map logits.
        """
        seg_logit = self.inference(img[0], img_metas[0], rescale, **kwargs)
    ...
    def aug_test_logits(self, img, img_metas, rescale=True, **kwargs):
        """Test with augmentations.
