...
class SegMANGeoFusionDecoder(BaseDecodeHead):
    ...
    def forward_winssm(self, x: torch.Tensor, c2, c3, c4, c1=None):
        ...
        if x.size(-1) % 4 != 0:
            ...
            x = F.pad(x, (0, pad_width, 0, 0))
        if x.size(-2) % 4 != 0:
            pad_height = (4 - (x.size(-2) % 4)) % 4
            x = F.pad(x, (0, 0, 0, pad_height))  # 在高度方向上填充
        ...
        if c2.size(-1) != _out_.size(-1):
            pad_width_c2 = (_out_.size(-1) - c2.size(-1)) % 4
            c2 = F.pad(c2, (0, pad_width_c2, 0, 0))
        if c2.size(-2) != _out_.size(-2):
            pad_height_c2 = (_out_.size(-2) - c2.size(-2)) % 4
            c2 = F.pad(c2, (0, 0, 0, pad_height_c2))
        if c3.size(-1) != _out_.size(-1):
            pad_width_c3 = (_out_.size(-1) - c3.size(-1)) % 4
            c3 = F.pad(c3, (0, pad_width_c3, 0, 0))
        if c3.size(-2) != _out_.size(-2):
            pad_height_c3 = (_out_.size(-2) - c3.size(-2)) % 4
            c3 = F.pad(c3, (0, 0, 0, pad_height_c3))
        if c4.size(-1) != _out_.size(-1):
            pad_width_c4 = (_out_.size(-1) - c4.size(-1)) % 4
            c4 = F.pad(c4, (0, pad_width_c4, 0, 0))
        if c4.size(-2) != _out_.size(-2):
            pad_height_c4 = (_out_.size(-2) - c4.size(-2)) % 4
            c4 = F.pad(c4, (0, 0, 0, pad_height_c4))
        c2 = c2 + _out_
        c3 = c3 + _out_
        c4 = c4 + _out_
    ...
    def forward_test(self, inputs, img_metas, test_cfg, geo_edge=None):
        return self.forward(inputs, geo_edge=geo_edge)

    def forward(self, inputs, geo_edge=None):
